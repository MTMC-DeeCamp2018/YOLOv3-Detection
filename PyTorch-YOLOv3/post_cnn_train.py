...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv2=nn.Conv2d(32,80,3)
        self.fc1=nn.Linear(80*14*6,50)
...
def loadtraindata():
    ...
    trainset = torchvision.datasets.ImageFolder(path,
                                                transform=transforms.Compose([
                                                    transforms.Resize((64, 32)),  # 将图片缩放到指定大小（h,w）或者保持长宽比并缩放最短的边到int大小
                                                    transforms.ToTensor()])
                                                )
    #print(trainset.target_transform)
...
def trainandsave():
    ...
    for epoch in range(40):  # 训练的数据量为5个epoch，每个epoch为一个循环
        ...
        torch.save(net, 'post_cnn_weights/net%d.pkl'%epoch)  # 保存整个神经网络的结构和模型参数
        torch.save(net.state_dict(), 'post_cnn_weights/net_params%d.pkl'%epoch)
        #if running_loss / 200 < 0.0001:
            #break
