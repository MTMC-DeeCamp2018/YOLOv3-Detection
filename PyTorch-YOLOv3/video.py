...
ap.add_argument('--conf_thres', type=float, default=0.7
...
ap.add_argument("-v", "--video",default="terrace2-c0.avi",
...
out = cv2.VideoWriter('out_2_c0_post.avi',cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), fps, size)
while True:
    index = index+1
    print(index)
    # grab the current frame
    (grabbed, frame) = camera.read()
    #if index <=21600 or index%3!=0:
        #continue
    #elif index>25200:
        #break
    #print(frame.shape)
    ...
    if detections is not None:
        ...
        for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
            ...
            frame_crop= frame[max(int(y1[0]), 0):int((y1 + box_h)[0]), max(int(x1[0]), 0):int((x1 + box_w)[0])]
            if post_filter(frame_crop)==0:
                continue
            cv2.rectangle(frame, (x1, y1), (x1 + box_w, y1 + box_h), (0, 255, 0), 1)
            file.write(str(index) + ' ' + str(x1.cpu().data.numpy()) + ' ' + str(y1.cpu().data.numpy()) + ' ' + str(
                box_w.cpu().data.numpy()) + ' ' + str(box_h.cpu().data.numpy()) + ' ' + str(
                cls_conf.cpu().data.numpy()) + '\n')
            #time.sleep(1)
            #break
